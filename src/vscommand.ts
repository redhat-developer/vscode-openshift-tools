...
export function execute<T>(command: (...args: T[]) => Promise<any> | void, ...params: T[]): Promise<any> {
    try {
...
export async function registerCommands(...modules:string[]): Promise<Disposable[]>  {
    ...
    return vsCommands.map((cmd) => {
        return commands.registerCommand(cmd.commandId, (...params) => execute(cmd.method, ...params));
    });
}

export function vsCommand(commandId: string): Function {
