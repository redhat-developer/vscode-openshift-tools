...
export async function activate(extensionContext: vscode.ExtensionContext): Promise<void> {
    ...
    OdoImpl.Instance.subject.subscribe(async (event) => {
        if (event.type === 'inserted' && event.data.contextValue === ContextType.COMPONENT) {
            const choice = await vscode.window.showInformationMessage(`Do you want to push new '${event.data.getName()}' Component?`, 'Push Now');
            if (choice === 'Push') {
            ...
            const choice = await vscode.window.showInformationMessage(`Do you want to push changes for '${event.data.getParent().getName()}' Component?`, 'Push Now');
            if (choice === 'Push') {
