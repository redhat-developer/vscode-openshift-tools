...
export class Component extends OpenShiftItem {
    ...
    static async del(treeItem: OpenShiftObject): Promise<string> {
        ...
        if (value === 'Yes') {
            return Progress.execFunctionWithProgress(`Deleting the Component '${component.getName()} '`, async () => {
                await Component.unlinkComponent(component);
    ...
    static async unlinkComponent(component: OpenShiftObject) {
        const compData = await Component.odo.execute(Command.getdescribeComponent(component.getParent().getParent().getName(), component.getParent().getName(), component.getName()));
        ...
        if (linkComponent) {
            Object.keys(linkComponent).forEach(async key => {
                await Component.odo.execute(Command.unlinkComponentTo(key, component.getName()));
            });
        }
    }
