    ...
    while (step !== undefined) {
        switch (step) {
            ...
            case Step.enterPassword: {
                ...
                const stored = await TokenStore.getItem('registry', tokenKey);
                ...
                if (stored) {
                    ...
                    if (loggedIn) {
                        ...
                        return { registry, username, password: stored };
                    }
                    try {
                        await ContainerRuntimeDetector.loginToRegistry(runtime, registry, username, stored);
                        await saveRegistrySettings(registry, username);
                        return { registry, username, password: stored };
                    } catch {
                        await TokenStore.setItem('registry', tokenKey, '');
                    }
                }
